...
```bash
git clone https://github.com/Trevo88423/motu-korikori.git
cd motu-korikori/dictionary-app
npm install
```
...
2. Go to SQL Editor and run the schema from `dictionary-app/supabase/schema.sql`
3. Copy your project URL and anon key

### 3. Set up Cloudflare R2

1. Create an R2 bucket called `motu-dictionary-audio`
2. Deploy the Worker from `dictionary-app/cloudflare-worker/`
...
```bash
# Generate seed SQL from CSV files
cd dictionary-app
npm run seed:generate

# Then run the generated SQL in Supabase SQL Editor:
# Copy contents of dictionary-app/supabase/seed.sql
...
├── LICENSE                   # MIT License
├── SELF_HOSTING.md           # Fork for another language
├── PROJECT_STRUCTURE.md      # Detailed structure
│
├── files/                    # Source data (15,610 words)
│   ├── motu_vocabulary.csv
│   └── aligned_verses.csv
│
├── align_bibles.py           # Bible alignment script
│
...
See [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) for detailed file listing.
